...
int main()
{
	...
	glm::mat4 projection = glm::ortho(-1.0f, 1.0f, -1.0f, 1.0f, 0.1f, 100.0f);//izquierda derecha abajo arriba z near y z far
	...
	while (!mainWindow.getShouldClose())
	{
		...
		//Inicializar matriz de dimensión 4x4 que servirá como matriz de modelo para almacenar las transformaciones geométricas
		model = glm::mat4(1.0);
		model = glm::translate(model, glm::vec3(2.0f, 0.0f, -4.0f));//mas lejos de Z, mas pequeños, mas cerca de Z, mas grandes
		model = glm::rotate(model, glm::radians(angulo), glm::vec3(0.0f, 1.0f, 0.0f));//estamos rotando alrededor del eje y
		//Si ponemos al reves la funcion translate y rotate, es decir, primero roto y luego traslado, el cubo esta rotando en el origen
